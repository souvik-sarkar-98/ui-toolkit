...
```bash
npm install @ssui-toolkit/auth-core @ssui-toolkit/auth-angular
```
...
import { authGuard, permissionGuard } from '@ssui-toolkit/auth-angular';
...
```bash
npm run build -w @ssui-toolkit/auth-angular
npm test -w @ssui-toolkit/auth-angular
```
