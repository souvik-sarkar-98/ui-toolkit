...
npm workspaces + [Turborepo](https://turbo.build/) of **shared frontend libraries** for Nabarun web clients. This repository publishes `@ssui-toolkit/*` packages; applications live in separate repos (for example the frontend monorepo) and consume these libraries from npm or a workspace link.

## Packages

| Family | Packages | Docs |
|--------|----------|------|
| Forms | `@ssui-toolkit/forms-core`, `forms-react`, `forms-angular` | [forms-core](packages/forms-core/README.md) · [forms-react](packages/forms-react/README.md) · [forms-angular](packages/forms-angular/README.md) |
| Comments | `@ssui-toolkit/comment-core`, `comment-react`, `comment-angular` | [comment-core](packages/comment-core/README.md) · [comment-react](packages/comment-react/README.md) · [comment-angular](packages/comment-angular/README.md) |
| List dashboard | `@ssui-toolkit/list-dashboard-core`, `list-dashboard-angular` | [list-dashboard-core](packages/list-dashboard-core/README.md) · [list-dashboard-angular](packages/list-dashboard-angular/README.md) |
| Auth | `@ssui-toolkit/auth-core`, `auth-angular` | [auth-core](packages/auth-core/README.md) · [auth-angular](packages/auth-angular/README.md) |
...
Target one package with `npm run <script> -w @ssui-toolkit/<name>`.

## Adding a shared package

Create `packages/<name>/package.json` (name `@ssui-toolkit/<name>`), then depend on it from another workspace with `"*"` or from an app via npm. Turborepo runs dependency builds first via `dependsOn: ["^build"]`. After a public API or behavior change, run `npm run changeset`.
...
| Group | Packages |
|-------|----------|
| Forms | `@ssui-toolkit/forms-core`, `@ssui-toolkit/forms-react`, `@ssui-toolkit/forms-angular` |
| Comments | `@ssui-toolkit/comment-core`, `@ssui-toolkit/comment-react`, `@ssui-toolkit/comment-angular` |
| List dashboard | `@ssui-toolkit/list-dashboard-core`, `@ssui-toolkit/list-dashboard-angular` |
| Auth | `@ssui-toolkit/auth-core`, `@ssui-toolkit/auth-angular` |

### CI release pipeline

GitHub Actions uses the **deploy-platform** reusable workflows (this workspace’s `deploy-main` repo, GitHub `nabarun-ngo/deploy-platform`). The publish workflow versions with Changesets and publishes `@ssui-toolkit/*` to the public npm registry. It does **not** create git tags or GitHub Releases. Version history lives in `package.json`, `CHANGELOG.md`, and npm.
...
|--------|----------------|---------|
| `main` | `latest` | `npm install @ssui-toolkit/auth-angular` |
| `stage` | `beta` | `npm install @ssui-toolkit/auth-angular@beta` |

**Setup (one-time):**

1. Add repository secret **`NPM_TOKEN`** with an npm automation token that can publish `@ssui-toolkit/*` packages.
2. Ensure you are logged in to npmjs with publish rights for `@ssui-toolkit` (`npm login`). Root [`.npmrc`](.npmrc) points the scope at `registry.npmjs.org`.
...
- Anyone can install without a token: `npm install @ssui-toolkit/auth-angular` (stable) or `npm install @ssui-toolkit/auth-angular@beta`.
- For CI publish, use an npm automation token (`NPM_TOKEN`) with write access to the `@ssui-toolkit` org.
